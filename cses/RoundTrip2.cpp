...
// actual solutions 
int solve(int u,VI &path ,vector<bool> &gvis,unordered_set<int> &pvis,VII &adj){
    gvis[u] = true;
    pvis.insert(u);
    path.push_back(u);
    trav(v,adj[u]){
        if(pvis.find(v)!=pvis.end()) return v;
        if(!gvis[v]){
            int cycle = solve(v,path,gvis,pvis,adj);
...
int main()
{
    ...
    FOR(i,0,N){
        if(!gvis[i]) {
            auto cycle = solve(i,path,gvis,pvis,adj);
            if(cycle!=-1){
                flag = true;
