...
int main()
{
    ...
    FOR(i, 0, M)
    {
        ...
        adj2[b - 1].push_back({a-1,w});
    }
    VI forwardDist(N,INF);
    VI reverseDist(N,INF);
    djisktra(0,adj,forwardDist);
    djisktra(N-1,adj2,reverseDist);
    LL res= INF;
    FOR(u,0,N){
        trav(ed,adj[u] ){
            res = min(res,forwardDist[u] + reverseDist[ed.first] + ed.second/2);
        }
    }
    cout << res << endl;
