...
// actual solutions TLE - 1 case
void djisktra(int src, vector<vector<PI>> &adj)
{
    int n = adj.size();
    VI distTo(n, INF);                              // store min distance
    VI distTo2(n, INF);                             // store min distance with discounted flight
    ...
    while (!pq.empty())
    {
        auto p = pq.top();
        pq.pop();
        if (distTo[p.second] < p.first)
            continue;
        if(p.second==n-1) break;
        trav(ed, adj[p.second])
        {
            distTo2[ed.first] = min(distTo2[ed.first], min(distTo[p.second] + ed.second / 2, distTo2[p.second] + ed.second));
            if (distTo[ed.first] > distTo[p.second] + ed.second)
            {
                distTo[ed.first] = distTo[p.second] + ed.second;
                pq.push({distTo2[ed.first], ed.first});
            }
        }
    }
    cout << distTo2[n - 1] << endl;
}
