...
int main()
{
    ...
    cin.tie(nullptr);
    vector<int> dp1(MAXN,MAXN);
    dp1[1] = 0;
    FOR(i,1,MAXN){
        FOR(x,1,i+1){
            int j = i + i/x;
            if(j<MAXN) dp1[j] = min(dp1[j],dp1[i]+1);
        }
    }
    int T;
    cin>>T;
    while(T--){
        ...
        take_vec(A,n);
        take_vec(C,n);
        int dp[n+1][k+1]; // knapack 
        memset(dp,0,sizeof dp);
        FOR(i,1,n+1){
            FOR(j,0,k+1){
                if(j<dp1[A[i-1]]) dp[i][j] = dp[i-1][j];
                else dp[i][j] = max(dp[i-1][j],dp[i-1][j-dp1[A[i-1]]] + C[i-1]);
            }
        }
        put(dp[n][k]);
