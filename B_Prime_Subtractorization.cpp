...
typedef valarray<CD> CC;
void fast_ft(CC& x) {
    const size_t N = x.size();
    if (N <= 1) return;
    CC even = x[slice(0, N / 2, 2)];
    CC odd = x[slice(1, N / 2, 2)];
    fast_ft(even);
    fast_ft(odd);
    for (size_t i = 0; i < N / 2; i++) {
        CD t = polar(1.0, -2 * PI * i / N) * odd[i];
        x[i] = even[i] + t;
        x[i + N / 2] = even[i] - t;
    }
}

void inverse_fft(CC& x) {
    x = x.apply(conj);
    fast_ft(x);
    x = x.apply(conj);
    x /= x.size();
