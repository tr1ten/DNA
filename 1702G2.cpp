...
void testcase(){
    ...
    rep(i,0,q){
        ...
        cin >> k;
        vi qs(k);
        rep(j,0,k) {cin >> qs[j];qs[j]--;}
        int lc = (qs[0]);
        int fst=qs[0];
        trav(j,qs){
            lc = lca(j,lc);
            if(depth[fst]<depth[j]) fst=j;
        }
        int scd=-1;
        trav(j,qs){
            if(lca(fst,j)!=j && (scd==-1 || depth[scd]<depth[j])) scd = j;
        }
        if(scd==-1) {pyn(1);continue;}
        int third=-1;
        trav(j,qs){
            if(lca(fst,j)!=j && lca(scd,j)!=j) third = j;
        }
        if(third!=-1) {pyn(0);continue;}
        pyn((path_dist(fst,scd)==path_dist(fst,lc) + path_dist(scd,lc) ));
