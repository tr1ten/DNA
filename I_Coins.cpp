...
int main()
{
    ...
    while(T--){
        ...
        take_vec(P,n);
        int n_2 = n/2;
        long double dp[n+1][n_2+1]={0};
        FOR(i,0,n_2+1) dp[0][i] = 0.0L;
        dp[0][0] = 1.0L;
        FOR(i,1,n+1){
            long double p = P[i-1];
            FOR(k,0,n_2+1){
                dp[i][k] = p*dp[i-1][k] + (k==0 ? 0 : (1-p)*dp[i-1][k-1]);
            }
        }
        long double res = reduce(dp[n],dp[n]+n_2+1,0.0L);
        std::cout<< std::setprecision(10) << res << endl;
    }
