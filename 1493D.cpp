...
void testcase(){
    ...
    rep(j,0,q){
        ...
        i--;
        debug(i,x);
        for(auto c:factors(x)){
            int gold = ms[c.first].size()==n ?  *ms[c.first].begin() : 0;
            int oldc = a[i][c.first];
            auto it =ms[c.first].find(oldc);
            if(it!=ms[c.first].end()) ms[c.first].erase(it);
            ms[c.first].insert(oldc+c.second);
            if((gold)==oldc && ms[c.first].size()==n && ms[c.first].count(oldc)==0){
                if(oldc){
                    int pp = fast_pow(c.first,oldc,MOD);
                    int inv= fast_pow(pp,MOD-2,MOD);
                    g = g*inv%MOD;
