...
#define put_vec(vec)               \
    trav(x, vec) cout << x << " "; \
    cout << endl;
#define put(x) cout <<setprecision(10)<< (x) << endl;
...
const LL INF = 1e10 + 5;
const int nmax = 301;
double ev[nmax][nmax][nmax];
int ac,bc,cc;
int n;
double f(int a,int b,int c){
    if(a<0 || b<0 || c<0 || (a==0 && b==0 && c==0)) return 0;
    if(ev[a][b][c]>=0) return ev[a][b][c];
    // if(a==0 && b==0 && c==0){}
    double res = n+ a*f(a-1,b,c) + b*f(a+1,b-1,c) + c*f(a,b+1,c-1);
    return ev[a][b][c] = res/(a+b+c);
}
// driver code
int main()
{
