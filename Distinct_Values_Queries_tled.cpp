...
const int N = 2*(1e5) + 5;
int n;
static set<ll> segm[2*N]; // 0 index
...
void query(int node,int left,int right,int l,int r,set<ll> &st){
    // propogate(node,left,right);
    if(left>=l && right<=r) return st.insert(all(segm[node]));
    int mid = (left+right)/2;
    if(l<=mid)  query(2*node+1,left,mid,l,r,st);;
    if(r>mid) query(2*node+2,mid+1,right,l,r,st);
