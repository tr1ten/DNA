...
#define less(a,b) a<b
#define vcnt(vec) accumulate(vec.begin(), vec.end(), 0L);
...
void propogate(int node,int left,int right){
    rep(i,0,A){
        if(segm[node].lazy&(1<<i)){
            segm[node].cnt[i] = (right-left+1-segm[node].cnt[i]);
        }
    }
    if(left<right){
...
void update(int node,int left,int right,int l,int r,ll value){
    ...
    if(left>=l && right<=r) {
        ...
        segm[node].cnt = combine(segm[2*node+1] , segm[2*node+2]).cnt;
...
void testcase(){
    cin >> n;
    rep(i,0,n) {int x;cin >> x;update(i,i,x); }
    int m;
    cin >> m;
    rep(i,0,m){
        int t;
