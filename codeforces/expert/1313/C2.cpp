...
void testcase(){
	...
	rep(i,0,n) {
		...
		if(st.size()) {
			st.push({i,st.top().second+ a[i]*(i-st.top().first)});
		}
		else {
			st.push({i,a[i]*(i+1)});	
		}
	...
	per(i,0,n) {
		...
		if(st.size()) {
			st.push({i,st.top().second+ a[i]*(st.top().first - i)});
		}
		else {
			st.push({i,a[i]*(n-i) });	
		}
	...
	rep(i,0,n) {
		// debug(lef[i],rig[i],i);
		res = max(res,{lef[i] + rig[i] - a[i],i});
	}
	assert(res.second!=-1);
	rr[res.second] = a[res.second];
	rep(i,res.second+1,n) {
		rr[i] = min(rr[i-1],a[i]);
	}
	per(i,0,res.second) {
		rr[i] = min(rr[i+1],a[i]);
