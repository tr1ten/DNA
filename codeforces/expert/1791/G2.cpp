...
void testcase(){
	...
	while(s1.size() && k>0) {
		...
		if((*s1.begin()).first<(*s2.begin()).first ) {
			...
			s1.erase(s1.find(s));
			l++;
			s2.erase(s2.find({a[s.second]+n-s.second,s.second} ));
			k-=s.first;
		}
		else{
			s = *s2.begin();
			...
			r++;
			vs.pb(s.first);
			mnd = min(mnd,(a[s.second]+1+s.second - s.first));
			s1.erase(s1.find({a[s.second]+1+s.second,s.second} ));
			k -=s.first;
		}
	}
	if(l==0) {
		...
		// debug(l,r,s1);
		exr = min(exr,(int)(lower_bound(all(vs),mnd-k)-vs.begin() +1) );
		trav(x,s1){
			exr = min(exr,(int)(lower_bound(all(vs),x.first-k)-vs.begin() ) );
		}
