...
void testcase(){
	...
	if(!(p[0]==1 && s.back()==n && s[0]==p.back())) {
		put(0);
		return;
	}
	ll res = nCk(n-1,s[0]-1);
	rep(i,0,m1-1){
		res = ((res*nCk(p[i+1]-2,p[i]-1))%MOD)*fact[p[i+1]-2-p[i]+1]%MOD;
	}
	per(i,1,m2){
		res = ((res*nCk(n-s[i-1]-1,n-s[i]))%MOD)*fact[n-s[i-1]-1 - (n-s[i])]%MOD;
	}
	put(res);
