...
void testcase(){
	...
	vi divs = divisors(mat[0][0]);
	vector<vector<vector<ll>>> dp(n,vii(m,vi(divs.size())));
	...
	rep(i,0,n){
		rep(j,0,m) {
			if(i+1<n){
				rep(k,0,divs.size()) {
					if(dp[i][j][k] && (mat[i+1][j]%divs[k])==0) dp[i+1][j][k] = 1;
				}
			} 
			if(j+1<m){
				rep(k,0,divs.size()) {
					if(dp[i][j][k] && (mat[i][j+1]%divs[k])==0) dp[i][j+1][k] = 1;
				}
			} 
		}
	}
	ll ans = 1;
	rep(i,0,divs.size()){
		if(dp[n-1][m-1][i]) ans = max(divs[i],ans);
	}
	put(ans);
}
// driver code
