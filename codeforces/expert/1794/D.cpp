...
void testcase(){
	...
	trav(x,c){
		a.push_back(x.first);
		prime.push_back(isprime(x.first));
	}
	...
	rep(i,1,m+1){
		rep(j,1,n+1){
			dp[i][j] = dp[i-1][j];
			if(prime[i-1]) {dp[i][j] = modadd(dp[i][j],modmul(dp[i-1][j-1],c[a[i-1]],MOD),MOD);}
		}
	}
	ll ways = modmul(fact[n],dp[m][n],MOD);
	rep(i,0,m) {
		if(c[a[i]]==1) continue;
		ways = modmul(ways,finv[c[a[i]]],MOD);
	}
	put(ways);
