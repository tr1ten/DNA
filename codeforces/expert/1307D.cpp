...
typedef long long ll;
const ll INF = 1e15;
vector<ll> dijsktra(int S,vector<vector<pi>> &adj){
    int n = adj.size();
...
void testcase(){
    ...
    vi d1 = dijsktra(0,adj);
    vi dn = dijsktra(n-1,adj);
    multiset<ll> ms;
    trav(x,a){
        ms.insert(d1[x-1]);
    }
    sort(all(a),[&] (int i,int j){
        return dn[i-1]<dn[j-1];
    });
    ll res =0;
    trav(x,a){
        ms.erase(ms.find(d1[x-1]));
        if(ms.size()) res = max(dn[x-1]+*ms.rbegin()+1,res);
    }
    // debug(res,d1[n-1]);
    put(min(res,d1[n-1]));
