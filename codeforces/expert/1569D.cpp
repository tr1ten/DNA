...
public:
    ...
    int sum(int i) {
        int r = 0;
        // i += 1not needed here since we need sum of rank less than i rank[0...i-1]
        while (i > 0) {
            r += nums[i];
            i -= (i & (-i));
        }
...
void testcase(){
    ...
    rep(i,0,k){
        int x,y;
        cin >> x >> y;
        if((xs.count(x)^ys.count(y))){ ppx[x].insert(y);ppy[y].insert(x);}
        
    }
    ll res = 0;
    trav(x,xs){
        trav(y,ppx[x]){
            res += xx.sum(*ys.upper_bound(y)) - xx.sum(*(--ys.upper_bound(y))+1);
        }
        trav(y,ppx[x]){
            xx.update(y,1);
        }
    ...
    trav(y,ys){
        trav(x,ppy[y]){
            res += xx.sum(*xs.upper_bound(x)) - xx.sum(*(--xs.upper_bound(x))+1);
        }
        trav(x,ppy[y]){
            xx.update(x,1);
        }
