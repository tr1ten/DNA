// Problem: F. Kirill and Mushrooms
// Contest: Codeforces - Codeforces Round 935 (Div. 3)
// URL: https://codeforces.com/contest/1945/problem/F
// Memory Limit: 256 MB
// Time Limit: 2000 ms
// 
...
void testcase(){
	...
	pi res = {0,0};
	rep(i,0,n){
		if((int)(st.size())<i+1) break;
		res = max(res,{(ll)(i+1)*(*(st.find_by_order(st.size()-1-i))),-(i+1)});
		// debug(st,res,i);
		st.erase((st.find_by_order(st.order_of_key(a[p[i]-1])) ));
	}
	put2(res.first,-res.second);
}
