...
auto res = new int[MXN][MX](); // huh? w/o () not work
...
int main()
{
    ...
    FOR(i,0,MX) res[mxl-1][i] = 0; // prefix sum
    FOR(j, mxl, mxr+1)
    {
        FOR(i, 0, MX)
        {
            res[j][i] = res[j-1][i]+ ((j & (1 << i)) > 0);
        }
    }
    trav(x,queries){
        int l=x.first,r=x.second;
        int rr[MX]={0};
        FOR(i,0,MX) {rr[i] = res[r][i]-res[l-1][i]; }
        put(r - l + 1 - *max_element(rr, rr + MX));
    }
