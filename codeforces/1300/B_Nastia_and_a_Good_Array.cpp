...
int main()
{
    ...
    while(T--){
        ...
        int minI = 0;
        FOR(i,0,n){
            if(vec[i]<vec[minI]) minI = i;
        }
        if(minI>0) {
            res.push_back({0,minI,vec[minI],vec[0]});
            swap(vec[0],vec[minI]);
            }
        FOR(i,1,n){
            res.push_back({0,i,vec[0],vec[i-1]+1});
            vec[i] = vec[i-1]+1;
        }
        FOR(i,1,n){
            if(gcd(vec[i],vec[i-1])!=1) {
                put3("Not good ",i,gcd(vec[i],vec[i-1]));
            }
        }
        put(res.size());
        // put_vec(vec);
        trav(x,res) {
            cout << (x[0]+1) << " " << (x[1]+1) << " " << x[2] << " "<< x[3] << endl;
        }
    }
