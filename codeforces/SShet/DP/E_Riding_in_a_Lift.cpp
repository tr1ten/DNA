...
int main()
{
    ...
    while(T--){
        ...
        LL pref[N+1];
        pref[0] = 0;
        FOR(i,1,N+1){pref[i] = pref[i-1] + (i!=B);}
        FOR(k,1,K+1){
            LL temp[N+1];
            temp[0] = 0;
            FOR(i,1,N+1){
                int d = abs(i-B)-1;
                temp[i] = (temp[i-1] + (-pref[max(1,i-d) - 1] +  pref[min(N,i+d)] + MOD)%MOD - ((pref[i]-pref[i-1] + MOD)%MOD) + MOD)%MOD; 
                // update(temp,i,(range(bit,max(1,i-d),min(N,i+d)) - range(bit,i,i) + MOD)%MOD,N+1);
            }
            memcpy(pref,temp,sizeof temp);
        }
        put((pref[A]-pref[A-1] + MOD)%MOD);
    }
