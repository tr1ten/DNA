...
ll dfs(int u,int p,unordered_map<ll,unordered_map<ll,ll>> &cnt){
    ll res = 0;
    trav(v,adj[u]){
        if(v==p) continue;
        cnt[u][v] += dfs(v,u,cnt);
...
int main(int argc, char const *argv[])
{
    ...
    rep(i,0,q){
        ...
        u--;v--;
        int lc = lca(u,v);
        if(u!=root) {cnt[u][up[u][0]]++;cnt[up[u][0]][u]++;}
        if(v!=root) {cnt[v][up[v][0]]++;cnt[up[v][0]][v]++;}
        if(lc!=root) {cnt[lc][up[lc][0]] -=2;cnt[up[lc][0]][lc] -=2;}
        // debug(lc);
    }
    dfs(root,-1,cnt);
    trav(e,edges){
