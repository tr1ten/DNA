...
int main()
{
    ...
    while(T--){
        ...
        ROF(i,0,n){
            PI top = pq.top();
            pq.pop();
            res[vec[i].second] = top.second+1;
            pq.push({top.first+vec[i].first,top.second});
        }
        PI mnq = pq.top();
        pq.pop();
        PI mxq=mnq;
        while(!pq.empty()) {mxq=pq.top();pq.pop();}
        if(mxq.first-mnq.first>x) {put("NO")continue;}
        put("YES");
        put_vec(res);
        // now n-m boxes left to place
