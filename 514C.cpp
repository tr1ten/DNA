...
int main()
{
    ...
    while(T--){
        int n,m;
        cin >> n >> m;
        unordered_map<int,unordered_map<pi,unordered_set<int>,pair_hash>> hashes;
        rep(i,0,n){
            string s;
            cin >> s;
            HashedString hs(s);
        ...
        rep(i,0,m){
            ...
            if(it1!=hashes[nn].end() && ((*it1).second.count((s[0]-'a'+1)%3) || (*it1).second.count((s[0]-'a'+2)%3))) f=1;
            it1 =  hashes[nn].find({hs.getHash(0,nn-2),0});
            if(it1!=hashes[nn].end() && ((*it1).second.count((s[nn-1]-'a'+1)%3) || (*it1).second.count((s[nn-1]-'a'+2)%3))) f=1;
            rep(j,1,nn-1){
                if(f) break;
                pi pp = {hs.getHash(0,j-1),hs.getHash(j+1,nn-1)};
                it1 =  hashes[nn].find(pp);
                if(it1!=hashes[nn].end() && ((*it1).second.count((s[j]-'a'+1)%3) || (*it1).second.count((s[j]-'a'+2)%3))) f=1;
            }
            if(f) put("YES")
            else put("NO")
