...
class BookMyShow:
    ...
    def __init__(this, n: int, m: int):
        ...
        for i in range(n):
            this.rq.update(i, m)  # add capacity of each row
        this.n = n
        this.m = m
    ...
    def gather(this, k: int, mr: int) -> List[int]:
        ...
        lw = this.rq.lower_max(0, this.rq.n-1, k, 1)  # lowest index where max>=k
        if (lw > mr):
            return []
        res = (lw, this.m-this.rq.max_tree[this.rq.n+lw])  # min_row,col
        this.rq.update(lw, -k)  # decrease capacity
        if(this.rq.max_tree[this.rq.n+lw]==0):
            this.zeroRow = lw
        return res

    def scatter(this, k: int, mr: int) -> bool:
        if (k > this.rq.query_sum(0, mr)):
            return False  # if total cap of <=mr rows is less than k
        i = this.zeroRow+1
        while (k > 0):
            cap = this.rq.tree[i+this.rq.n]
            this.rq.update(i, -min(k, cap))
            k -= min(k, cap)
            if (this.rq.max_tree[i+this.rq.n] == 0):
                this.zeroRow = i
            i += 1
        return True
...
class RangeQuery:
    ...
    def _query_sum(self, ns: int, ne: int, start: int, end: int, node: int) -> int:
        ...
        if (ne < start or end < ns):
            return 0  # doesn't lie in the range
        mid = (ns+ne)//2
    ...
    def _query_max(self, ns: int, ne: int, start: int, end: int, node: int) -> int:
        ...
        if (ne < start or end < ns):
            return float('-inf')  # doesn't lie in the range
        mid = (ns+ne)//2
        left, right = self._childs(node)
