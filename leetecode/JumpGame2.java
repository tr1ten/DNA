...
class Solution {
     public int maxResult(int[] nums, int k) {
        ...
        pq.add(mem[n-1]);
        for(int i=n-2;i>=0;i--){
            mem[i] = nums[i]+pq.peek();
            pq.add(mem[i]);
            if(pq.size()>k) pq.remove(mem[i+k]);
        }
        return mem[0];
    }
}
class JumpGame2 {
    public int maxResult(int[] nums, int k) {
        ...
        int[] mem = new int[n];
        mem[n-1] = nums[n-1];
        IndexMaxPQ<Integer> pq = new IndexMaxPQ<>(n);
        pq.insert(n-1,mem[n-1]);
        for(int i=n-2;i>=0;i--){
            mem[i] = nums[i]+pq.maxKey();
            if(pq.size()>=k) pq.delete(i+k);
            pq.insert(i,mem[i]);
        }
        return mem[0];
    }
