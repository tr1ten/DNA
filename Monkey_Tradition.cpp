...
bool solve(int i){
    ...
    int lcm = n1*(n2/g);
    int lcm2 = n2/g;
    int x = modadd((modmul(n1,modmul(x1,diff/g,lcm2),lcm)),a1,lcm);
    cgs[i].ff = x;
    cgs[i].ss = lcm;
    return solve(i+1);
...
void testcase(int i){
    ...
    cgs.resize(n);
    rep(i,0,n){
        cin >> cgs[i].ss >> cgs[i].ff;
        cgs[i].ff %=cgs[i].ss;
    }
    if(!solve(1)) cout << "Case " << i+1<< ": " << "IMPOSSIBLE" << endl;
    else cout << "Case " << i+1<< ": " << cgs.back().ff << endl;
}
// driver code
int32_t main()
{
