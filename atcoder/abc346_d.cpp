...
// do not use unordered map use 2mll
const int N = 2*(1e5) + 5;
ll dp[N][2][2];
void testcase(){
	int n;
	...
	rep(i,1,n+1){
		int z = s[i-1]=='1';
		dp[i][z][0] = dp[i-1][!z][0];
		dp[i][!z][0] = dp[i-1][z][0] + c[i-1];
		if(i==1) continue;
		dp[i][z][1] = min(dp[i-1][!z][1],dp[i-1][z][0]);
		dp[i][!z][1] = min(dp[i-1][!z][0],dp[i-1][z][1]) + c[i-1];
	}
	put(min(dp[n][0][1],dp[n][1][1]) ) ;
}
// driver code
int main()
{
