...
void testcase(){
    ...
    mll ind;
    rep(i,0,b.size()) {ind[b[i]]=i;}
    FenwickTreeMax fw(b.size());
    vi A;
    int r = 0;
    for(int i=0;i<a.size();i++){
        int j = ind[a[i]];
        int mx = j==0 ? 0 : fw.getmax(j-1);
        if(mx+1>fw.getmax(j)) fw.update(j,mx+1);
        A.pb(mx+1);
        r = max(r,mx+1);
    }

    vector<set<int>> nxt(r+2);
    vi res;
    per(i,0,m){1` z
        auto it = nxt[A[i]+1].upper_bound(a[i]);
        if(r==A[i] || it!=nxt[A[i]+1].end()) {
            res.pb(i+1);
            nxt[A[i]].insert(a[i]);
        }
