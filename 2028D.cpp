...
void testcase(){
    cin >> n;
    vii a(3,vi(n));
    tkv(a[0],n);tkv(a[1],n);tkv(a[2],n);
    BIT dp0(n),dp1(n),dp2(n);
    vector<BIT> dps = {dp0,dp1,dp2};   
    vector<int> rmp[3];
    rep(j,0,3){
        rmp[j].resize(n);
        rep(i,0,n){
    ...
    per(i,0,n){
        if(i==n-1){
            ...
            rep(j,0,3){
                if(a[j][i]==1 || dps[j].sum(a[j][i]-1)==0) continue;
                int lo = 0,hi = a[j][i]-2;
                int bi = hi;
                while (lo<=hi)
                {
    ...
    if(bt[0].first!=-1){
        ...
        while (cur!=n-1)
        {
            rrs.push_back({bt[cur].second,bt[cur].first+1});
            cur = bt[cur].first;
        }
        put(rrs.size());
        trav(x,rrs){
            put2(sk[x.first],x.second);
        }
