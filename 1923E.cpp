...
int merge(int a,int b,int d){
    if(cnts[a].size() < cnts[b].size()) swap(a,b);
    for(auto v:cnts[b]) {
        if(d!=v.ff) {
            ans += v.ss*(cnts[a][v.ff]);
        }
        cnts[a][v.ff] += v.ss;
    }
    return a;
...
int dfs(int u,int p){
    ...
    for(auto v:adj[u]){
        if(v!=p){
            childs.push_back(dfs(v,u));
            cur_set=merge(cur_set,childs.back(),c[u]);
        }
    }
    ans += cnts[cur_set][c[u]];
    cnts[cur_set][c[u]] = 1;
    return cur_set;
}
